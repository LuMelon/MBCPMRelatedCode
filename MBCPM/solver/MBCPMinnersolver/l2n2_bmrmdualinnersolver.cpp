     ...
     staleness(0)
{
   ...
   gradIdleAge = 10;
   // if(config.IsSet("clock_table_staleness")){
   //    gradIdleAge=config.GetInt("clock_table_staleness");
   // }
   // num_workers=config.GetInt("num_clients")*config.GetInt("num_worker_threads")-1;
   ...
   if(config.IsSet("L2N2_BMRMDualInnerSolver.diagonalPDMatrix"))
   {
      ...
      while(!feof(ifp))
      {
         ...
         if(tmp <= 0)
            throw MBCPMException("Diagonal PD Matrix should be strictly positive\n","CL2N2_BMRMDualInnerSolver::CL2N2_BMRMDualInnerSolver()");
         tmpB.push_back(tmp);
      }
      fclose(ifp);
   ...
   GradInfo.resize(maxGradSetSize,0);
   ...
   b = new double(1.0); 
   if(config.IsSet("staleness")){
      staleness=config.GetInt("staleness");
   }
   int trainningIter=config.GetInt("trainning_iter");
   IdxOfIter.resize(trainningIter+1);
   IterOfIdx.resize(trainningIter+1);
   // initializations
   Reset();
}
...
void CL2N2_BMRMDualInnerSolver::SinkGradSet(double * ScalingVec,double totalScale,std::vector<bool> &sink){
   int idx=0;
   double scaler=0.0;
   int sinkIter=0;
   ostringstream oss;
   ...
   for (int i =0; i <dim; ++i)
   {
      if (x[i]>0)
      {
         sinkIter=IterOfIdx[i];
         oss<<sinkIter<<", ";
         sink[sinkIter]=true;
         scaler=ScalingVec[sinkIter]/totalScale;
         gradientSet[i]->Scale(scaler);
         int theRow = i*dim;
         for(int j=0; j < dim; j++) 
         {
            Q[theRow + j] = Q[theRow+j]*scaler;
         }
         for(int j=0; j < dim; j++)
         {
            Q[j*dim + i] =Q[j*dim+i]*scaler;
         }      
         offsetSet[i]*=scaler;
         f[i] = -offsetSet[i]*QPScale;
         x[i]=0;  
      }
   }
   oss<<"]";
...
void CL2N2_BMRMDualInnerSolver::ChangeGradSet(TheMatrix& a,double b,int iter_idx,int inc)
{
   double b_val=b;
   int idx=IdxOfIter[iter_idx];
   if (enterTimeStamp[idx]!=iter_idx+1){
       printf("\nChangeGrad:wrong deleted gradient,staleness=%d,dim=%d,iter_idx=%d, idx=%d, addr=%x\n",staleness,dim,iter_idx,idx,gradientSet[idx]);
       throw MBCPMException("did't find gradient","CModel::ChangeGradSet()");  
   }
   gradType = SML::DENSE;
   if(a.Density() < SPARSE_TO_DENSE_THRESHOLD)
      gradType = SML::SPARSE;
   if(gradientSet[idx]){
      // gradientSet[idx]->Print();
     gradientSet[idx]->copy_thematrix(a,gradType);
   }
   
   offsetSet[idx]=b_val;
   
   activeTimeStamp[idx] = iter+1; 
   GradInfo[idx]=inc;

   // if (inc==num_workers){
   //    assert(iter_idx+1>left);
   //    left=iter_idx+1;
   // }
   ...
   {  
      ...
      if(a.Length() != B->Length())
      {
         ...
         throw MBCPMException(oss.str(), "CL2N2_BMRMDualInnerSolver::Update()");
      }
...
void CL2N2_BMRMDualInnerSolver::AddGradSet(TheMatrix& a,double b,int iter_idx,int inc)
{
   double b_val=b;
   int idx=IdxOfIter[iter_idx];
   if (enterTimeStamp[idx]!=iter_idx+1){
       printf("\nAddGrad:wrong deleted gradient,staleness=%d,dim=%d,iter_idx=%d, idx=%d, addr=%x\n",staleness,dim,iter_idx,idx,gradientSet[idx]);
       throw MBCPMException("did't find gradient","CModel::ChangeGradSet()");  
   }
...
void CL2N2_BMRMDualInnerSolver::Update(TheMatrix& a, double b)
{
   iter++;
   int idx = 0;
   prevDim = dim;//when did dim being change
   //dim means how many gradient
   
   if(removeAllIdleGrad)
      idx = RemoveAllIdleGradients(); //how idle gradient be defined
   else
   ...
   IdxOfIter[iter-1]=idx;
   IterOfIdx[idx]=iter-1;
...
int CL2N2_BMRMDualInnerSolver::AggregateGradients()
{
   ...
   //why it could be changed? can't find information in this function
   ...
   for(int i=0; i<dim; i++)
   {
      ...
      if (enterTimeStamp[i]>left)continue;
...
int CL2N2_BMRMDualInnerSolver::RemoveLaziestIdleGradient()
{
 //  int idx = 0;
 //  
 //  for(int i=0; i < dim; i++) //find the smallest(laziest) timestamp index,
 //  {
 //     if (enterTimeStamp[i]>left)
 //     {
 //        continue;
 ...
 //  if(left - activeTimeStamp[idx] < gradIdleAge) 
 //  {
 //     idx = dim;
 //     dim++;
 //  }
...
void CL2N2_BMRMDualInnerSolver::SolveW(TheMatrix &w,double &objval)
{
   SolveQP();
   GetSolution(w, objval);
