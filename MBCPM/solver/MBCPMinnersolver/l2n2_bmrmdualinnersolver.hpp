 ...
 */
class CL2N2_BMRMDualInnerSolver : public MBCPMInnerSolver 
{
   protected:
       ...
       */
      int prevDim;
      /** parameters in ssp table,default:0
      */
      int staleness;
      ...
      */
      ...
      std::vector<int> IdxOfIter;
      std::vector<int> IterOfIdx;
   ...
   public:     
       ...
       */
      void Update(TheMatrix& a, double b);
      ...
      /** reset staleness
      */
      inline void resetStaleness(int newVal){
            staleness=newVal;
      }

      inline void resetlineCnt(int newVal){
            lineCnt=newVal;
      }
      /** slow node's grad submitted to update the gradient set 
      */
      void AddGradSet(TheMatrix &a, double b,int iter_idx,int count);

      /** update the exisited gradient set 
      */
      void ChangeGradSet(TheMatrix &a, double b,int iter_idx,int count);

      void SinkGradSet(double * ScalingVec,double totalScale, std::vector<bool> &sink);

      /** Solve the problem
       */
      virtual void Solve(TheMatrix& w, TheMatrix& a, double loss, double &objval);
      /**Solve the problem with the exsiting Gradient Set
      */
      void SolveW(TheMatrix &w,double &objval);
