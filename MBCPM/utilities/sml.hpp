...
public:
  ...
  TheMatrix* CreateMatrixColView(const unsigned int& index);  // return a new TheMatrix "pointing" the index-th row of *this
  void CreateMatrixColView(const unsigned int& index, TheMatrix* mat_row);  // Assign a reference "pointing" the index-th row of *this to mat_row

  // assign pointers
  void assign_pointers();
  void copy_thematrix(const TheMatrix& tm, const int& _dense_or_sparse);
  ublas::compressed_matrix<double>* GetSparseMatrix(){return sm;};
...
inline int TheMatrix::getClock() const{
  return clock;
}
inline void TheMatrix::setClock(int value){
  clock=value;
  return;
}
inline int TheMatrix::getFlag() const{
  return exitflag;
}
inline void TheMatrix::setFlag(int value){
  exitflag=value;
  return;
}
/** 
