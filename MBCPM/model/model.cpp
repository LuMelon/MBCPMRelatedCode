...
void CModel::Initialize(const string &fname)
{
   ...
   if(! fp.good()) 
   {
      throw MBCPMException("Cannot open file <" + fname + ">\n","CModel::Initialize()");
   }   
...
void CModel::Load(const string &fname)
{
   if(_w == 0)
   {
      MBCPMException("weight vector has not been initialized yet!","CModel::Load()");
   }      
   ifstream fp(fname.c_str());
   if(! fp.good()) 
   {
   ...
   if(in_numOfW != _numOfW) 
   {
      ...
      oss << "number of W in model (" << in_numOfW 
          ...
          << _numOfW << ")";
      throw MBCPMException(oss.str(),"CModel::Load()");
   }
...
void CModel::Save(const string &fname)
{
   ...
   if(! outfp.good())
   {
      ...
      throw MBCPMException(oss.str(),"CModel::Save()");      
   }
