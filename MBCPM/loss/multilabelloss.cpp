     ...
     g(0)          
{
   ...
   if(_data->HasLabel() && (_data->MinLabel() < 1.0)) 
      throw MBCPMException("Labels must be positive integers","CMultilabelLoss::CMultilabelLoss()");
...
void CMultilabelLoss::Evaluate(CModel* model)
{
   ...
   if(! _data->HasLabel())
   {
      throw MBCPMException("Data object does not contain labels","CMultilabelLoss::Evaluate()");
   }
