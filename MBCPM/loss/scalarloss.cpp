...
void CScalarLoss::InitStochasticSolver(int batchsize){
   Index=(int* )malloc(batchsize*sizeof(int));
   samplef = new TheMatrix(batchsize, 1, SML::DENSE);
   samplel = new TheMatrix(batchsize, 1, SML::DENSE); 
   _data->CreateFeatureMatrixRowViews(); 
}

void CScalarLoss::StochasticComputeLossAndGradient(double& loss, TheMatrix& grad, int batchsize){
   TheMatrix &w = _model->GetW();
   loss=0;
   f->Zero();
   ...
   //_data->Grad(*samplel,grad);
   grad.Scale(1.0/batchsize); 
}

int CScalarLoss::GetDataSize(){
    return m;
}
