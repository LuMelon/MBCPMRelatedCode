     ...
     _indloss(0)
{
   ...
   {
      if(! data->HasLabel())
         throw MBCPMException("Error: dataset contains no labels; unable to determine number of classes!",
   ...
   {
      throw MBCPMException("Labels must be positive integers",
                           "CWTAMulticlassLoss::CWTAMulticlassLoss()");
   }
...
void CWTAMulticlassLoss::InitStochasticSolver(int batchsize){
   Index=(int* )malloc(batchsize*sizeof(int));
}

void CWTAMulticlassLoss::StochasticComputeLossAndGradient(double& loss, TheMatrix& grad, int batchsize){
   TheMatrix &w = _model->GetW();
   double y_i = 1;
   double ybar_i = 1;
   double f_y_i = 0;
   ...
   StochasticIndex(Index,batchsize,_data->size());
   // _data->StochasticComputeF(w,Index,batchsize,*samplef);
   // StochasticLossAndGrad(loss, *samplef, *samplel,Index,batchsize);
   // loss = loss*(1.0/batchsize);
   // _data->ComputeStochasticGrad(*samplel,grad,Index,batchsize);
   int i=0;
   for(unsigned int batch_i=0; batch_i<batchsize; batch_i++) 
   {
      i=Index[batch_i];
      _data->ComputeFi(*matW, *f, i);
      Y.Get(i, y_i);
...
void CWTAMulticlassLoss::Evaluate(CModel* model)
{
   ...
   {
      throw MBCPMException("Data object does not contain labels","CWTAMulticlassLoss::Evaluate()");
   }
...
int CWTAMulticlassLoss::GetDataSize()
{
    return m;
}
