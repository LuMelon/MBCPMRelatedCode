 ...
 */
void CVecFeature::LoadFeatureMatrix(unsigned int start, unsigned nparts,std::string filename)
{
    assert(start <= nparts); 
...
void CVecFeature::ScanFeatureFile()
{
    ...
    if(!featureFp.good()) 
    {
        ...
        throw MBCPMException(msg, "CVecFeature::ScanFeatureFile()");
    }
...
//                                        throw MBCPMException(ostr.str(), "CVecFeature::ScanFeatureFile()");
//                                }
            ...
            fflush(stdout);
            printf("\rlines:%d",numOfAllExample);
        }
        printf("\n");
    }
...
void CVecFeature::LoadFeatures()
{
    ...
    {    
        ...
        throw MBCPMException(msg, "CVecFeature::LoadFeatures()");
    }
    ...
    if(featureType==DENSE_FEATURE){
        ...
        memset(val, 0, sizeof(double)*featureDimension);
    }
    else{
    ...
    {                                  
        ...
        {      
            ...
            fflush(stdout);
            printf("\r%d|%d",lineCnt,numOfExample);
            addrowtime.Stop();
...
void CVecFeature::StochasticComputeF(const TheMatrix& w, int* index, int batchsize, TheMatrix& f)
{
    int type=X->Type();
    // double* other=w.Data();
    double* output=f.Data();
    int cols=X->Cols();
    // assert(type==SML::DENSE_MATRIX);
    assert(cols==w.Cols());
    for (int i = 0; i < batchsize; ++i){
        x[index[i]]->Dot(w,output[i]);
    }
}
...
void CVecFeature::ComputeStochasticGrad(const TheMatrix& l, TheMatrix& grad, int* indexs, int batchsize)
{
    // double* mtx=X->Data();
    // int cols=X->Cols();
    // double* grad_ptr=grad.Data();
    double* l_ptr=l.Data();
    // if(featureType == DENSE_FEATURE){
    //     for (int i = 0; i < cols; ++i){
    //         grad_ptr[i]=0;
    //         for (int j = 0; j < batchsize; ++j){
    //             grad_ptr[i]+=l_ptr[j]*mtx[indexs[j]*cols+i];
    //         }
    //     }
    // }
    // else{
    //     ublas::compressed_matrix<double>* sm=X->GetSparseMatrix();
    //     for (int i = 0; i < cols; ++i){
    //         grad_ptr[i]=0;
    //         for (int j = 0; j < batchsize; ++j){
    //             grad_ptr[i]+=l_ptr[j]*((*sm)(indexs[j],i));
    //         }
    //     }
    // }
    for(int i=0;i<batchsize;i++){
        grad.ScaleAdd(l_ptr[i],*x[indexs[i]]);
    }
