...
SGD::SGD(CModel *model, CLoss* loss, int _procID, int _nProc)
   : CSolver(model, loss),
     verbosity(0),
     maxNumOfIter(10000),
     epsilonTol(-1.0),
...
double SGD::GetCurrentWallClock(){
 ...
 */
void SGD::ConfirmProgramParameters()
{
   ...
   if(config.IsSet("BMRM.maxNumOfIter")) 
   {
      ...
      if(maxNumOfIter < 0)
         throw SGDException("BMRM.maxNumOfIter must be > 0\n","SGD::ConfirmProgramParameters()");
   }
   ...
   if(epsilonTol<0.0 && relEpsilonTol<0.0 && gammaTol<0.0 && relGammaTol<0.0)
      throw SGDException("At least one of the BMRM.{epsilonTol, gammaTol, relEpsilonTol, relGammaTol} must be > 0\n","SGD::ConfirmProgramParameters()");
   
   if(config.IsSet("BMRM.lambda"))           
   {
      lambda = config.GetDouble("BMRM.lambda");
      if(lambda <= 0)
         throw SGDException("BMRM.lambda must be > 0\n","SGD::ConfirmProgramParameters()");
   }
   ...
   if(config.IsSet("BMRM.checkpointInterval")) 
   {
      ...
      if(checkpointInterval < 1)
         throw SGDException("BMRM.checkpointInterval must be a positive integer!\n","SGD::ConfirmProgramParameters()");
   }
   ...
   if (config.IsSet("BatchSize"))
      ...
      throw SGDException("No BatchSize Defined!\n","SGD::ConfirmProgramParameters()\n");
   ...
   if (config.IsSet("TestLabelFile"))
      ...
      throw SGDException("Cannot find test label file\n","SGD::ConfirmProgramParameters()\n");
   if (config.IsSet("TestFeatureFile"))
      TestFeatureFile=config.GetString("TestFeatureFile");
   else
      throw SGDException("Cannot find test Feature file\n","SGD::ConfirmProgramParameters()\n");
...
void SGD::DisplayIterationInfo(unsigned int iter, double exactObjVal, double approxObjVal, 
...
void SGD::SaveCheckpointModel(unsigned int iter)
{
...
int SGD::CheckTermination(unsigned int iter, double epsilon, double gamma, 
...
void SGD::AdjustInnerSolverOptTol(double& innerSolverTol, double prevEpsilon, double epsilon)
{
}


void SGD::DisplayAfterTrainingInfo(unsigned int iter, double finalExactObjVal, 
