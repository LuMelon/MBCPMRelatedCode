...
void CModel::Initialize(const string &fname)
{
   ...
   if(! fp.good()) 
   {
      throw AdamException("Cannot open file <" + fname + ">\n","CModel::Initialize()");
   }   
...
void CModel::Load(const string &fname)
{
   if(_w == 0)
   {
      AdamException("weight vector has not been initialized yet!","CModel::Load()");
   }      
   ifstream fp(fname.c_str());
   if(! fp.good()) 
   {
   ...
   if(in_numOfW != _numOfW) 
   {
      ...
      oss << "number of W in model (" << in_numOfW 
          ...
          << _numOfW << ")";
      throw AdamException(oss.str(),"CModel::Load()");
   }
...
void CModel::Save(const string &fname)
{
   ...
   if(! outfp.good())
   {
      ...
      throw AdamException(oss.str(),"CModel::Save()");      
   }
