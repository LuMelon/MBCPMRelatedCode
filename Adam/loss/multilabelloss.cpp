     ...
     g(0)          
{
   ...
   {
      if(! data->HasLabel())
         throw AdamException("Error: dataset contains no labels; unable to determine number of classes!",
   ...
   if(_data->HasLabel() && (_data->MinLabel() < 1.0)) 
      throw AdamException("Labels must be positive integers","CMultilabelLoss::CMultilabelLoss()");
...
void CMultilabelLoss::Evaluate(CModel* model)
{
   ...
   if(! _data->HasLabel())
   {
      throw AdamException("Data object does not contain labels","CMultilabelLoss::Evaluate()");
   }
