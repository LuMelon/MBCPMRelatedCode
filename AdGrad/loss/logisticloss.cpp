...
void CLogisticLoss::StochasticLossAndGrad(double& loss, TheMatrix& f, TheMatrix& l, int* indexs, int batchsize){
    l.Zero();  // for gradient computation i.e. grad := l'*X
    double* Y=NULL;
    Y=_data->labels().Data();
    double* f_array=f.Data();
    for (int i = 0; i < batchsize; ++i)
        f_array[i]=f_array[i]*Y[indexs[i]];
    ...
    double* l_array=l.Data();
    for (int i = 0; i < batchsize; ++i){
      l_array[i]*=Y[indexs[i]];
    }   
}
