...
Adgrad::Adgrad(CModel *model, CLoss* loss, int _procID, int _nProc)
   : CSolver(model, loss),
     verbosity(0),
     maxNumOfIter(10000),
     epsilonTol(-1.0),
...
void Adgrad::UpdateModel(double* model,double* multiplier,double* gradient, double residual, int len){
   for (int i = 0; i < len; ++i){
      model[i]=model[i]-LearningRate*gradient[i]/(sqrt(multiplier[i]+residual));
   }
}

double Adgrad::GetCurrentWallClock(){
...
void Adgrad::Train()
{
   ...
   do
   {
      iter++;
      {
         ...
         multiplier.Add(grad);
         UpdateModel(w.Data(),multiplier.Data(),a.Data(),residual,row*col);
 ...
 */
void Adgrad::ConfirmProgramParameters()
{
   ...
   if(config.IsSet("BMRM.maxNumOfIter")) 
   {
      ...
      if(maxNumOfIter < 0)
         throw AdgradException("BMRM.maxNumOfIter must be > 0\n","Adgrad::ConfirmProgramParameters()");
   }
   ...
   if(epsilonTol<0.0 && relEpsilonTol<0.0 && gammaTol<0.0 && relGammaTol<0.0)
      throw AdgradException("At least one of the BMRM.{epsilonTol, gammaTol, relEpsilonTol, relGammaTol} must be > 0\n","Adgrad::ConfirmProgramParameters()");
   
   if(config.IsSet("BMRM.lambda"))           
   {
      lambda = config.GetDouble("BMRM.lambda");
      if(lambda <= 0)
         throw AdgradException("BMRM.lambda must be > 0\n","Adgrad::ConfirmProgramParameters()");
   }
   ...
   if(config.IsSet("BMRM.checkpointInterval")) 
   {
      ...
      if(checkpointInterval < 1)
         throw AdgradException("BMRM.checkpointInterval must be a positive integer!\n","Adgrad::ConfirmProgramParameters()");
   }
   ...
   if (config.IsSet("BatchSize"))
      ...
      throw AdgradException("No BatchSize Defined!\n","Adgrad::ConfirmProgramParameters()\n");
   ...
   if (config.IsSet("TestLabelFile"))
      ...
      throw AdgradException("Cannot find test label file\n","Adgrad::ConfirmProgramParameters()\n");
   if (config.IsSet("TestFeatureFile"))
      TestFeatureFile=config.GetString("TestFeatureFile");
   else
      throw AdgradException("Cannot find test Feature file\n","Adgrad::ConfirmProgramParameters()\n");
...
void Adgrad::DisplayIterationInfo(unsigned int iter, double exactObjVal, double approxObjVal, 
...
void Adgrad::SaveCheckpointModel(unsigned int iter)
{
...
int Adgrad::CheckTermination(unsigned int iter, double epsilon, double gamma, 
...
void Adgrad::AdjustInnerSolverOptTol(double& innerSolverTol, double prevEpsilon, double epsilon)
{
}


void Adgrad::DisplayAfterTrainingInfo(unsigned int iter, double finalExactObjVal, 
