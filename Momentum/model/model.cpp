...
void CModel::Initialize(const string &fname)
{
   ...
   if(! fp.good()) 
   {
      throw MomentumException("Cannot open file <" + fname + ">\n","CModel::Initialize()");
   }   
...
void CModel::Load(const string &fname)
{
   if(_w == 0)
   {
      MomentumException("weight vector has not been initialized yet!","CModel::Load()");
   }      
   ifstream fp(fname.c_str());
   if(! fp.good()) 
   {
   ...
   if(in_numOfW != _numOfW) 
   {
      ...
      oss << "number of W in model (" << in_numOfW 
          ...
          << _numOfW << ")";
      throw MomentumException(oss.str(),"CModel::Load()");
   }
...
void CModel::Save(const string &fname)
{
   ...
   if(! outfp.good())
   {
      ...
      throw MomentumException(oss.str(),"CModel::Save()");      
   }
