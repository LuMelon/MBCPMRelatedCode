...
Momentum::Momentum(CModel *model, CLoss* loss, int _procID, int _nProc)
   : CSolver(model, loss),
     verbosity(0),
     maxNumOfIter(10000),
     epsilonTol(-1.0),
...
double Momentum::GetCurrentWallClock(){
...
void Momentum::Train()
{
   ...
   do
   {
      iter++;
      {
         ...
         grad.Scale(momentum);
         grad.Add(a);
         w.ScaleAdd(-1*LearningRate,grad);
 ...
 */
void Momentum::ConfirmProgramParameters()
{
   ...
   if(config.IsSet("BMRM.maxNumOfIter")) 
   {
      ...
      if(maxNumOfIter < 0)
         throw MomentumException("BMRM.maxNumOfIter must be > 0\n","Momentum::ConfirmProgramParameters()");
   }
   ...
   if(epsilonTol<0.0 && relEpsilonTol<0.0 && gammaTol<0.0 && relGammaTol<0.0)
      throw MomentumException("At least one of the BMRM.{epsilonTol, gammaTol, relEpsilonTol, relGammaTol} must be > 0\n","Momentum::ConfirmProgramParameters()");
   
   if(config.IsSet("BMRM.lambda"))           
   {
      lambda = config.GetDouble("BMRM.lambda");
      if(lambda <= 0)
         throw MomentumException("BMRM.lambda must be > 0\n","Momentum::ConfirmProgramParameters()");
   }
   ...
   if(config.IsSet("BMRM.checkpointInterval")) 
   {
      ...
      if(checkpointInterval < 1)
         throw MomentumException("BMRM.checkpointInterval must be a positive integer!\n","Momentum::ConfirmProgramParameters()");
   }
   ...
   if (config.IsSet("BatchSize"))
      ...
      throw MomentumException("No BatchSize Defined!\n","Momentum::ConfirmProgramParameters()\n");
   if (config.IsSet("Momentum"))
      momentum=config.GetDouble("Momentum");
   else
      throw MomentumException("No Momentum parameter Defined", "Momentum::ConfirmProgramParameters()\n");
   ...
   if (config.IsSet("TestLabelFile"))
      ...
      throw MomentumException("Cannot find test label file\n","Momentum::ConfirmProgramParameters()\n");
   if (config.IsSet("TestFeatureFile"))
      TestFeatureFile=config.GetString("TestFeatureFile");
   else
      throw MomentumException("Cannot find test Feature file\n","Momentum::ConfirmProgramParameters()\n");
...
void Momentum::DisplayIterationInfo(unsigned int iter, double exactObjVal, double approxObjVal, 
...
void Momentum::SaveCheckpointModel(unsigned int iter)
{
...
int Momentum::CheckTermination(unsigned int iter, double epsilon, double gamma, 
...
void Momentum::AdjustInnerSolverOptTol(double& innerSolverTol, double prevEpsilon, double epsilon)
{
}


void Momentum::DisplayAfterTrainingInfo(unsigned int iter, double finalExactObjVal, 
