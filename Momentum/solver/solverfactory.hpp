   ...
   public:
      ...
      {         
         ...
         if(config.IsSet("Solver.type"))
         {
            ...
            { 
              printf("Get BMRM\n");
               solver = new Momentum(model, loss, procID, nProc);
               printf("after Get BMRM\n");
            }
            ...
            {
               throw MomentumException("ERROR: unrecognised solver ("+solverType+")\n", "CSolverFactory::GetSolver()");
            }
         } 
         else
         {
            throw MomentumException("ERROR: no solver specified!\n", "CSolverFactory::GetSolver()");
         }
