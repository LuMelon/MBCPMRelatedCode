...
class CDataFactory
{
   public:
       ...
       *   @return data object
       */
      static CData* GetData(unsigned int start=0, unsigned int nparts=1,std::string featurefile="", std::string labelfile="")
      {  
         CData *ds = 0;
         ...
         {
            ds = new CVecData(start,nparts,featurefile,labelfile);            
         }
         ...
         {
            throw MomentumException("ERROR: unrecognised data format ("+dataFormat+")\n", "CDataFactory::GetData()");
         }
