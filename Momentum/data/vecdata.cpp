...
/**  Constructor
 */
CVecData::CVecData(unsigned int start, unsigned int nparts,std::string f_filename,std::string l_filename)
   : CData(), 
     CVecFeature(), 
     CVecLabel()
{
        LoadFeatureMatrix(start, nparts,f_filename);
        LoadLabelMatrix(startExampleIndex, numOfExample,l_filename);
        ...
        if(numOfAllExample != numOfAllLabel)
        {
                ...
                throw MomentumException(msg.str(),"CVecData::CVecData()");
        }
