...
/** Load label matrix
 */
void CVecLabel::LoadLabelMatrix(unsigned int startlabel, unsigned int nlabels,std::string filename)
{
...
{  
   ...
   if(!labelFp.good()) 
   {
      ...
      throw MomentumException(msg, "CVecLabel::ScanLabelFile()");
   }
      ...
      {       
         ...
         else 
         {
            ...
            ostr << "Label file contains invalid label at line " << numOfAllLabel+1 << "!\n" 
                 << "Token: " << token << "\n";
            throw MomentumException(ostr.str(), "CVecLabel::ScanLabelFile()");
         }
      }
   ...
   if(numOfAllLabel <= 0) 
      throw MomentumException("Label set is empty!","CVecLabel::ScanLabelFile()");
...
void CVecLabel::LoadLabels()
{
   ...
   {
      string msg = "Cannot open label file <" + labelFile + ">!";
      throw MomentumException(msg, "CVecLabel::LoadLabels()");
   }
