...
{ 
    ...
    if(!featureFp.good()) 
    {
        ...
        throw MomentumException(msg, "CVecFeature::ScanFeatureFile()");
    }
...
//                                        throw MomentumException(ostr.str(), "CVecFeature::ScanFeatureFile()");
//                                }
...
void CVecFeature::LoadFeatures()
{
    ...
    {    
        ...
        throw MomentumException(msg, "CVecFeature::LoadFeatures()");
    }
...
void CVecFeature::StochasticComputeF(const TheMatrix& w, int* index, int batchsize, TheMatrix& f)
{
    ...
    {
        data=mtx+cols*index[i];
        output[i]=0;
        for (int j = 0; j < cols; ++j)
        {
            output[i]=output[i]+data[j]*other[j];
        }
    }
